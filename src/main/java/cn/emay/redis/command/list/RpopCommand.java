 ...
 * @author Frank
 */
public class RpopCommand<K> implements RedisCommand<K> {
    ...
    public RpopCommand(String key, Class<K> clazz, String datePattern) {
        this.key = key;
        this.clazz = clazz;
    ...
    private K exec(JedisCommands command, BinaryJedisCommands bjcommand, BinaryJedisClusterCommands bjccommand) {
        if (byte[].class.isAssignableFrom(clazz)) {
            byte[] value = null;
            if (bjcommand != null) {
                value = bjcommand.rpop(key.getBytes(StandardCharsets.UTF_8));
            } else if (bjccommand != null) {
                value = bjccommand.rpop(key.getBytes(StandardCharsets.UTF_8));
            }
            return (K) value;
        } else if (String.class.isAssignableFrom(clazz)) {
            return (K) command.rpop(key);
        } else {
            String value = command.rpop(key);
