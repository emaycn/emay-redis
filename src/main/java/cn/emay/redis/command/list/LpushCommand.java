 ...
 * @author Frank
 */
public class LpushCommand implements RedisCommand<Long> {
    ...
    public LpushCommand(String key, int expireTime, String datePattern, Object... objects) {
        this.key = key;
    ...
    private Long exec(JedisCommands command, BinaryJedisCommands bjcommand, BinaryJedisClusterCommands bjccommand) {
        ...
        if (!strvalues.isEmpty()) {
            length += command.lpush(key, strvalues.toArray(new String[0]));
        }
        if (!bytvalues.isEmpty()) {
            if (bjcommand != null) {
                length += bjcommand.lpush(key.getBytes(StandardCharsets.UTF_8), bytvalues.toArray(new byte[bytvalues.size()][]));
            } else if (bjccommand != null) {
                length += bjccommand.lpush(key.getBytes(StandardCharsets.UTF_8), bytvalues.toArray(new byte[bytvalues.size()][]));
            }
        }
        if (expireTime > 0) {
