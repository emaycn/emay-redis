	...
	private Long exec(JedisCommands command, BinaryJedisCommands bjcommand, BinaryJedisClusterCommands bjccommand) {
		...
		try {
			...
			if (!strvalues.isEmpty()) {
				result = command.sadd(key, strvalues.toArray(new String[strvalues.size()]));
			}
			if (!bytvalues.isEmpty()) {
				if (bjcommand != null) {
					result = bjcommand.sadd(key.getBytes("UTF-8"), bytvalues.toArray(new byte[bytvalues.size()][]));
				} else if (bjccommand != null) {
					result = bjccommand.sadd(key.getBytes("UTF-8"), bytvalues.toArray(new byte[bytvalues.size()][]));
				}
			}
			if (expireTime > 0) {
