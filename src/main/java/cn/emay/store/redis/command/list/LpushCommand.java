	...
	private Long exec(JedisCommands command, BinaryJedisCommands bjcommand, BinaryJedisClusterCommands bjccommand) {
		...
		try {
			...
			if (!strvalues.isEmpty()) {
				length += command.lpush(key, strvalues.toArray(new String[strvalues.size()]));
			}
			if (!bytvalues.isEmpty()) {
				if (bjcommand != null) {
					length += bjcommand.lpush(key.getBytes("UTF-8"), bytvalues.toArray(new byte[bytvalues.size()][]));
				} else if (bjccommand != null) {
					length += bjccommand.lpush(key.getBytes("UTF-8"), bytvalues.toArray(new byte[bytvalues.size()][]));
				}
			}
			if (expireTime > 0) {
